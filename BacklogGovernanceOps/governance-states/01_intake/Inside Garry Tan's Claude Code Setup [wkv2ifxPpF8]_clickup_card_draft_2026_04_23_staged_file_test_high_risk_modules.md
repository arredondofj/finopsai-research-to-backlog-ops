...
- The staged-file routing expansion should stay deterministic and file-path-based rather than becoming a heuristic or model-driven test selector.
...
- The mapping logic and the documentation need to stay aligned so developers can predict which test subset will run from staged paths.
...
FinOpsAI's repo-managed pre-commit unit-test subset is currently too narrow for the repository's documented verification expectations. `scripts/git/checks/unit_tests.sh` presently routes only a limited staged-file set to `tests/test_classification.py`, while repo guidance in `/Users/Sites/Repo-FinOpsAI/AGENTS.md` and `/Users/Sites/Repo-FinOpsAI/docs/engineering/testing.md` already identifies a broader focused accounting-sensitive subset including normalization, classification, controller validation, and Wave-related tests.

Expand staged-file test routing for high-risk FinOpsAI modules so the pre-commit harness runs the right focused tests when staged changes touch high-risk accounting and workflow areas.

Expected Outcome

The pre-commit unit-test routing becomes more complete and predictable for high-risk staged changes. When staged files affect the targeted high-risk modules, the hook runs the appropriate focused test subset instead of relying on the current classification-only mapping.

The resulting routing remains deterministic, scoped, and fast enough for pre-commit use.

Business Context

This is an internal engineering-quality improvement for a production-oriented bookkeeping repository. It supports better local verification of accounting-sensitive changes without changing application runtime behavior, transaction semantics, or posting behavior.

Scope

In scope:
- expand staged-file-to-test mapping in the repo-managed pre-commit unit-test check for high-risk modules
...
- bundling this work with the separate pre-commit summary-output card
- broad redesign of all pre-commit checks beyond the staged-file routing required for this card

Acceptance Criteria

1. `scripts/git/checks/unit_tests.sh` routes staged changes in the approved high-risk module set to the appropriate focused pytest subset rather than only to `tests/test_classification.py`.
2. The expanded mapping remains deterministic and based on explicit staged-path rules.
3. The expanded mapping covers the high-risk accounting/workflow areas identified by repo guidance where relevant, including the focused subset documented for normalization, classification, controller validation, and Wave-related behavior.
...
6. The governing documentation in `/Users/Sites/Repo-FinOpsAI/docs/engineering/git_pre_commit_implementation.md` and any aligned testing guidance is updated to reflect the implemented mapping behavior.
7. Focused tests are added or updated as needed to verify the routing behavior itself without inventing unrelated new scope.
...
1. Generate Specification
   - Confirm and refine the governing specification in `/Users/Sites/Repo-FinOpsAI/docs/engineering/git_pre_commit_implementation.md` for staged-file routing across the approved high-risk module set.
...
2. Generate Execution Prompt
   ...
   - The execution prompt should require focused script/test/doc updates only for this staged-file routing work item.

3. Implement deterministic staged-file routing expansion
   - Update the pre-commit unit-test routing logic for the approved high-risk module set.
...
5. Update documentation
   - Align `/Users/Sites/Repo-FinOpsAI/docs/engineering/git_pre_commit_implementation.md` and any relevant testing guidance with the final implemented routing behavior.
